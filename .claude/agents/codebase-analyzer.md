...
• **Python**: Entry points → Services → Models, follow async/await chains
• **CLI (Typer)**: Commands → Handlers → Services, trace command flow
• **Docker**: Compose files → Services → Environment config
...
**Example**: `Grep pattern="def.*function" glob="**/*.py" -C 3`
...
### Entry Points
• `file.py:45` - endpoint/function
• `handler.py:12` - main handler

### Implementation

**Validation** (`file.py:15-32`)
• ✓ Input validation check
• ✓ Type checking
• ✗ Returns error on failure

**Processing** (`service.py:8-45`)
• Parse payload (line 10)
• Transform data (line 23)
• Queue async (line 40)

### Data Flow
1. `cli/main.py:45` → 2. `handlers/command.py:12` → 3. `services/processor.py:8`

### Patterns
• **Factory**: `factories/processor.py:20`
• **Repository**: `stores/data-store.py`

### Config/Errors
• Config: `config/settings.py:5`
• Error handling: `handlers/command.py:28`
• Retry: `services/processor.py:52`
```
