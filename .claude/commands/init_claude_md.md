...
1. **Project structure** - Use codebase-locator to find:
   - Package manager (pyproject.toml, setup.py, etc.)
...
4. **Build/test commands** - Find:
   - Build scripts in pyproject.toml/Makefile/etc.
   - Test commands
...
2. **Identify project-specific customizations**:
   - Update verification commands (e.g., `poetry run pytest` → `make test`)
...
- Add command in `src/cow_performance/cli/`
- Create service in `src/cow_performance/`
- Add tests in `tests/unit/`

### For Load Generation Strategies:
- Implement in `src/cow_performance/load_generation/`
...
For a Python performance testing project:
...
Performance testing suite for CoW Protocol. Generates load, captures metrics, and benchmarks solver performance in a forked Ethereum environment.

**Tech**: Python 3.11+, Poetry, Docker Compose, Typer CLI
...
- `src/cow_performance/cli/` - CLI commands
